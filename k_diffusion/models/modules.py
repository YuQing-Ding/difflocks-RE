...
def _natten_uses_heads_last_layout():
    """Support both legacy and current NATTEN APIs.

    Older NATTEN versions expose `has_fused_na()` and only use the heads-last
    `na2d` fast path when fused kernels are available. Newer NATTEN releases
    removed that helper and expose a unified `na2d` API with the heads-last
    tensor layout by default.
    """
    if natten is None:
        return False

    has_fused_na = getattr(natten, "has_fused_na", None)
    if callable(has_fused_na):
        return has_fused_na()
...
class NeighborhoodSelfAttentionBlock(nn.Module):
    ...
    def forward(self, x, pos, cond):
        ...
        if _natten_uses_heads_last_layout():
